...
public class SkillGraphService
{
    ...
    /// <summary>
    /// Upserts the 7 Unit 3 skill nodes from BuildSkillSeed into gt_skills.
    /// Safe to call on every startup.
    /// </summary>
    public void MigrateUnit3Skills()
    {
        var conn = _db.GetConnection();
        var unit3Skills = BuildSkillSeed()
            .Where(s => s.Id.StartsWith("geo-u3-", StringComparison.Ordinal))
            .ToList();

        using var tx = conn.BeginTransaction();
        try
        {
            foreach (var s in unit3Skills)
            {
                using var ins = conn.CreateCommand();
                ins.Transaction = tx;
                ins.CommandText = """
